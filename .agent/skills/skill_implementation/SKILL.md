...
If CLAP was selected during `/plan`, ensure:
1. `clap-juce-extensions` submodule is added
2. CMakeLists.txt has CLAP extension enabled (see template)
3. CLAP-specific features (per-note modulation) are implemented if needed

**CLAP-specific parameter features:**
```cpp
// CLAP supports per-note parameter modulation
// Use JUCE's standard APVTS — clap-juce-extensions handles the bridge automatically
// No changes needed in your DSP code for basic CLAP support
```

---

## 🎛️ PRESET SYSTEM INTEGRATION

After DSP is functional, integrate the preset system:
1. Implement `getStateInformation()` / `setStateInformation()` (Phase 4.5)
2. Add PresetManager class (see `skill_presets/SKILL.md`)
3. Wire preset browser to UI (WebView or Visage)
4. Create factory presets based on creative brief categories

---

## 🤝 COLLABORATION MODULE (Optional)

If collaboration was planned:
1. Add `juce_osc` to CMakeLists.txt
2. Implement CollaborationManager (see `skill_collaboration/SKILL.md`)
3. Wire collaboration panel to UI
4. Test with two plugin instances on same network
...
Before shipping:
1. Generate marketplace metadata (see `skill_marketplace/SKILL.md`)
2. Create screenshots of plugin UI
3. Write product description
4. Set up installer scripts (InnoSetup for Windows, pkgbuild for macOS)
...
9. **Preset compatibility** - Version tag all saved state for forward compatibility
10. **CLAP compliance** - Test in CLAP hosts (Bitwig, Reaper) if CLAP enabled
